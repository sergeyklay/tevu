...
/**
 * Builds the contender-independent task prompt from only the allowed task and
 * source fields: configured prompt, task description, acceptance-criterion
 * descriptions, Definition of Done descriptions, the pinned source commit
 * identity, and the stay-inside-the-repository instruction. The same input
 * always yields the same bytes, so every contender receives an identical
 * prompt.
 */
export function buildTaskPrompt(task: TaskDefinition, sourceCommit: string): string {
