...
function buildConfig(repositoryPath: string, commit: string, overlayDirectory: string): TevuConfig {
  const config: TevuConfigInput = {
    version: 1,
    run: { output_dir: join(testDirectory, "artifacts"), concurrency: 1, timeout: "30s", stop_grace: "1s" },
  ...
  return {
    ...
    async run(input) {
      ...
      if (setupOutput !== SETUP_OUTPUT_CONTENT) {
        throw new Error(`expected before_agent's output to be visible to the agent, got: ${setupOutput}`);
      }
      await writeFile(join(worktree, "src/model-edit.txt"), `${MODEL_EDIT_BASE}${MODEL_EDIT_APPEND}`);
...
function buildHostProbe(): HostProbe {
  return { platform: "linux", nodeVersion: process.version, bunVersion: "0.0.0-synthetic", gitVersion: "2.45.0-synthetic" };
}
...
describe("repository setup orchestration end to end (AC-1, P7)", () => {
  it("keeps before_agent output out of the patch, includes the agent's edits, repairs a corrupted setup file before checks, and runs before_checks after restore and overlay (AC-1, P7)", async () => {
    ...
    const git = createGitWorkspaceAdapter({ config, workspacesDirectory: join(testDirectory, "workspaces") });
