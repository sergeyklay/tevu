...
import type { SetupCommand } from "../config/schema.ts";
import type { EvaluatorProcessAdapter, EvaluatorProcessRequest, EvaluatorProcessResult, RedactedCapture } from "../domain/types.ts";
import type { SetupPhaseInput } from "./repository-setup.ts";
...
function buildProcesses(run: (request: EvaluatorProcessRequest) => Promise<EvaluatorProcessResult>): EvaluatorProcessAdapter {
  return { run };
}
...
describe("runSetupPhase", () => {
  describe("classification", () => {
    it("classifies a launch that never started while cancelled as cancelled, with no error", async () => {
      ...
      const outcome = await runSetupPhase(buildInput({ processes, cancellation: controller.signal }));

      expect(outcome.status).toBe("cancelled");
      expect(outcome.commands).toEqual([
        { phase: "before_agent", argv: ["/synthetic/setup"], exitCode: null, durationMs: null, outcome: "cancelled" },
      ]);
    });

    it("classifies a launch that never started while not cancelled as launch-failed", async () => {
      const processes = buildProcesses(async () => ({ launched: false, reason: "ENOENT: not found" }));

      const outcome = await runSetupPhase(buildInput({ processes }));

      expect(outcome.status).toBe("failed");
      expect(outcome.commands).toEqual([
        { phase: "before_agent", argv: ["/synthetic/setup"], exitCode: null, durationMs: null, outcome: "launch-failed" },
      ]);
    ...
    it.each([{ aborted: false }, { aborted: true }])(
      ...
      async ({ aborted }) => {
        ...
        const processes = buildProcesses(async () => {
          ...
          return buildLaunched({ exitCode: null, signal: "SIGTERM", timedOut: true, terminationStage: "graceful" });
        });
    ...
    it("classifies exit code 0 as passed", async () => {
      ...
      expect(outcome.commands).toEqual([
        { phase: "before_agent", argv: ["/synthetic/setup"], exitCode: 0, durationMs: 10, outcome: "passed" },
      ]);
    });
    ...
    it("classifies a launched command with no exit code and not cancelled as failed by signal", async () => {
      const processes = buildProcesses(async () => buildLaunched({ exitCode: null, signal: "SIGSEGV" }));
  ...
  it("stops at the first command that does not pass and never starts the next one", async () => {
    const runSpy = vi.fn(async (request: EvaluatorProcessRequest) =>
      request.argv[0] === "/synthetic/first" ? buildLaunched({ exitCode: 1 }) : buildLaunched({ exitCode: 0 }),
    );

    const outcome = await runSetupPhase(
      buildInput({ commands: [["/synthetic/first"], ["/synthetic/second"]], processes: buildProcesses(runSpy) }),
    );
  ...
  it("builds a passing phase's log with one header, status, stdout, and stderr section ending in a newline", async () => {
    const processes = buildProcesses(async () =>
      buildLaunched({ stdout: buildCapture({ text: "built\n", totalBytes: 6 }), stderr: buildCapture() }),
    );

    const outcome = await runSetupPhase(buildInput({ commands: [["npm", "ci"]], processes }));

    expect(outcome.log).toBe('$ ["npm","ci"]\nexit code 0\nstdout (6 bytes):\nbuilt\nstderr (0 bytes):\n');
  });

  it("joins two commands' log sections with one blank line", async () => {
    const runSpy = vi.fn(async () => buildLaunched());

    const outcome = await runSetupPhase(buildInput({ commands: [["npm", "ci"], ["npm", "test"]], processes: buildProcesses(runSpy) }));
