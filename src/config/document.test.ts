...
import { appendToConfigText, renderConfigDocument } from "./document.ts";
...
import type { GitWorkspaceAdapter, TevuError, TevuResult } from "../domain/types.ts";
...
describe("renderConfigDocument", () => {
  ...
  it("redacts every decoded string value before it becomes YAML", () => {
    const config = buildConfig({ run: { output_dir: "SECRET_PATH", concurrency: 2, timeout: "10m", stop_grace: "3s" } });
...
describe("appendToConfigText", () => {
  const newTask = buildTask({
    ...
    checks: { acceptance: [buildManualCheck({ id: "a1" })], done: [buildManualCheck({ id: "d1" })] },
  });
  ...
  it("inserts the new task right after the last item when nothing follows it", () => {
    const result = appendToConfigText(EXISTING_TASK_LF, { task: newTask }, { redact: (value) => value });
  ...
  it("preserves the blank line separating the list from the next top-level section", () => {
    ...
    const result = appendToConfigText(text, { task: newTask, repository: newRepository }, { redact: (value) => value });
        ...
        findings: [
          {
            severity: "error",
            identifier: "tasks",
            message: "task add appends only to a block-style list; rewrite tasks in block style and run task add again",
          },
        ],
      },
    });
...
describe("createTask leaves the configuration file byte-identical on failure", () => {
  ...
  it("rejects a repo naming no configured or new repository", async () => {
    const input: TaskWizardInput = { configPath, task: buildTask({ id: "new-task", repo: "ghost" }) };
    ...
    const dependencies = buildTaskDependencies({
      git: {
        validateSource: async () => ({
          ok: false,
          error: { kind: "SourceMaterializationError", taskId: "substituted", reason: "commit not found" },
        }),
      } satisfies Pick<GitWorkspaceAdapter, "validateSource">,
    });
  ...
  it("returns CancellationError with no read when already cancelled", async () => {
    ...
    const result = await createTask(input, buildTaskDependencies({ cancellation: cancellation.signal }));
