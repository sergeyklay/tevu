...
import { Document, isSeq, parseDocument, Scalar, YAMLSeq } from 'yaml';
...
import type { TevuResult } from '@/domain/types';
...
export function renderConfigDocument(
  ...
  entries.push(
    [
      'repositories',
      config.repositories.map((repository) => buildRepositoryNode(repository, guard)),
    ],
...
export function appendToConfigText(
  ...
  const repositoryLines =
    addition.repository === undefined
      ? null
      : renderItemLines(buildRepositoryNode(addition.repository, guard));
  const failure = guard.failure();
  if (failure !== null) {
    return failure;
  }
  ...
  if (!isSeq(tasksSeq)) {
    return artifactFailure(
      ...
      'tasks is not a sequence node in the existing configuration',
    );
  }
  ...
  const failedInsertion = insertions.find(
    (insertion): insertion is Extract<(typeof insertions)[number], { ok: false }> => !insertion.ok,
  );
  ...
  const successfulInsertions = insertions
    .filter(
      (insertion): insertion is Extract<(typeof insertions)[number], { ok: true }> => insertion.ok,
    )
  ...
  return {
    redact(value) {
      ...
      if (typeof (redacted as unknown) !== 'string') {
        failure = artifactFailure(
          'render-configuration',
          'redaction returned no text while rendering the configuration',
        );
        return value;
      }
      return redacted;
...
function buildAgentsNode(
  agents: TevuConfigInput['agents'],
  guard: GuardedRedactor,
): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(agents).map(([name, settings]) => [
      name,
      buildAgentSettingsNode(settings, guard),
    ]),
  );
}
...
function findInsertionOffset(
  text: string,
  lastItemRange: readonly [number, number, number],
  dashColumn: number,
): number {
...
function buildInsertion(
  ...
  if (firstItem === undefined || lastItem === undefined) {
    return artifactFailure(
      'append-configuration',
      'the target list has no items to anchor the insertion against',
    );
  }
  ...
  if (firstRange === null || lastRange === null) {
    return artifactFailure(
      'append-configuration',
      "the target list's items carry no source position",
    );
  }
