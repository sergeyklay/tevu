...
import { decodeRunConfig } from './run-snapshot';

import type { CheckRecord, TaskRecord } from '@/domain/types';
...
describe('decodeRunConfig', () => {
  ...
  it('projects every model as id, model, and effort', () => {
    const decoded = expectDecoded(
      decodeRunConfig(
        buildSnapshot({ models: [buildSnapshotModel({ id: 'beta', agent: 'fake-agent' })] }),
      ),
    );
  ...
  it('projects every repository as id and path', () => {
    const decoded = expectDecoded(
      decodeRunConfig(
        buildSnapshot({
          repositories: [buildSnapshotRepository({ id: 'repo-9', path: '/repos/9' })],
        }),
      ),
    );
      ...
      tasks: [
        {
          ...
          checks: {
            acceptance: [
              { ...buildSnapshotCheck({ id: 'acc-1' }), timeout: '5m', exit_codes: [0], env: [] },
            ],
            done: [buildSnapshotCheck({ id: 'dod-1' })],
          },
        },
  ...
  it.each([
    {
      ...
      snapshot: buildSnapshot({
        repositories: [buildSnapshotRepository({ id: 'repo-[REDACTED]' })],
      }),
    },
    {
      fragment: 'url',
        ...
        tasks: [
          buildSnapshotTask({
            source: {
              kind: 'jira',
              key: 'PROJ-1',
              url: 'https://jira.example.com/browse/[REDACTED]',
            },
          }),
        ],
      }),
  ...
  it('names the root when the snapshot is not an object at all', () => {
    ...
    expect(result).toEqual({
      ...
      error: {
        ...
        operation: 'decode-run-configuration',
        reason:
          'run configuration snapshot does not match the current configuration layout at (root)',
      },
    });
  });
});
